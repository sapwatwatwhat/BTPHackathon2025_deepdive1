...
## 目次
- [デモ動画](#デモ動画) - AI Agentの動作と機能を紹介する映像資料
- [概要](#概要) - BTP上のAI Agentアプリケーションの構成とアーキテクチャ
- [CAP（知識DBへのデータの出し入れを担当）](#cap知識dbへのデータの出し入れを担当) - ナレッジベース管理のバックエンド部分
  - [1. アプリをデプロイする](#1-アプリをデプロイする) - MTAを使用したBTPへのアプリケーションデプロイ
  - [2. GETリクエストをテストする](#2-getリクエストをテストする) - CAPサービスのODataエンドポイント動作確認
  - [3. LLM のAPIをセットアップする](#3-llm-のapiをセットアップする) - AIモデルへの接続設定
  - [4. CAPの環境変数（.cdsrc.json）にLLM APIのIDを転記する](#4-capの環境変数cdsrcjsonにllm-apiのidを転記する) - LLM接続情報の設定ファイル更新
  - [5. 新しい環境変数で再デプロイする](#5-新しい環境変数で再デプロイする) - 更新した設定でのアプリケーション再構築
  - [6. 動作を確認する](#6-動作を確認する) - データ登録とベクトル化の検証
- [Python（LangChainフレームワークを用いた AI Agent）](#pythonlangchainフレームワークを用いた-ai-agent) - インテリジェントなAIエージェントの実装
  - [7. テスト環境の準備と実行](#7-テスト環境の準備と実行) - AIエージェントのテストと動作確認
- [まとめ](#まとめ) - 学習内容の振り返りと応用可能性
